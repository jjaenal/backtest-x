...
class BacktestResultViewModel extends BaseRefreshableViewModel {
  ...
  // Membuat teks ringkasan untuk dibagikan (memuat nama strategi dari StorageService)
  Future<String> _generateSummaryText() async {
    final summary = result.summary;
    // Resolve strategy name from storage (cached), fallback to id label
    String displayName = 'Strategy ${result.strategyId}';
    try {
      final storage = locator<StorageService>();
      final strategy = await storage.getStrategy(result.strategyId);
      if (strategy != null && strategy.name.trim().isNotEmpty) {
        displayName = strategy.name;
      }
    } catch (_) {
      // Ignore storage errors, fallback already set
    }
    final deeplink =
        locator<DeepLinkService>().buildBacktestResultLink(resultId: result.id);

    return '''
📊 Backtest Results - $displayName
...
Generated by BacktestX
  ...
  Future<void> shareResults() async {
    final rawText = await _generateSummaryText();
    final safeText = ShareContentHelper.sanitizeText(
      ShareContentHelper.redactPII(rawText),
    );
    try {
      final share = locator<ShareService>();
      await share.shareText(safeText, subject: 'BacktestX Results');
    } catch (e) {
      // Fallback to clipboard, prefer injected service if available
      if (locator.isRegistered<ClipboardService>()) {
        await locator<ClipboardService>().copyText(safeText);
      } else {
        await Clipboard.setData(ClipboardData(text: safeText));
      }
      _snackbarService.showSnackbar(
  ...
  // Copy deep link to this backtest result to clipboard
  Future<void> copyResultLinkToClipboard() async {
    try {
      final deepLinks = locator<DeepLinkService>();
  ...
  Future<void> exportPdf() async {
    ...
    try {
      ...
      final fileName = generateExportFilename(
        baseLabel: t.backtestReportFilenameLabel,
        ext: 'pdf',
      );
      final share = locator<ShareService>();
      await share.shareBytes(
        ...
        text: t.pdfShareBacktestReportText,
      );
      ...
      _snackbarService.showSnackbar(
        message: t.pdfExportFailed(e.toString()),
        duration: const Duration(seconds: 3),
      );
    } finally {
      setBusy(false);
