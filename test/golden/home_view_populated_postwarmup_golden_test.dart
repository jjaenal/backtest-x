...
void main() {
  ...
  testGoldens('HomeView - populated after warm-up complete (banner hidden)',
    ...
    // Prepare DataManager cache: disable background warm-up and mark as completed
    final dm = locator<DataManager>();
    dm.setBackgroundWarmupEnabled(false);
    dm.warmupNotifier.value = false; // ensure banner hidden
    ...
    // Pump beberapa frame agar layout stabil
    ...
    await expectLater(
      ...
      matchesGoldenFile('goldens/home_view_populated_postwarmup.png'),
    );
  });
}
